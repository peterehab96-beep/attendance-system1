...
/*
 * External Attendance Grading Management
 * This is the new system for admins to manage grading criteria and student reports
 */
...
export default function ExternalAttendanceGrading() {
  ...
  const loadAdminData = async () => {
    try {
      ...
      console.error("[External Grading] Error loading admin:", error)
      router.push('/simple-auth/admin/login')
    }
  }
  ...
  const loadGradingCriteria = async () => {
    try {
      ...
      const savedCriteria = localStorage.getItem('external_grading_criteria')
      if (savedCriteria) {
        setGradingCriteria(JSON.parse(savedCriteria))
      }
    } catch (error) {
      console.warn("[External Grading] Could not load grading criteria:", error)
    }
  }
  ...
  const loadStudentReports = async () => {
    try {
      ...
      console.error("[External Grading] Error loading reports:", error)
  ...
  const addGradingCriteria = () => {
    ...
    localStorage.setItem('external_grading_criteria', JSON.stringify(updatedCriteria))
  ...
  const goBack = () => {
    router.push('/external-attendance/admin')
  }
