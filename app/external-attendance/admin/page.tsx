...
/*
 * External Attendance Admin Dashboard
 * This is the new system for admins to generate QR codes that work with native camera apps
 */
...
export default function ExternalAttendanceAdmin() {
  ...
  const [currentSession, setCurrentSession] = useState<ExternalSession | null>(null)
  ...
  const loadAdminData = async () => {
    try {
      ...
      if (localAdmin) {
        ...
        setAdmin(adminData)
        console.log("[External Admin] Loaded admin:", adminData.name)
      } else {
        // Redirect to login if no admin data
        router.push('/simple-auth/admin/login')
      }
    } catch (error) {
      console.error("[External Admin] Error loading admin:", error)
      router.push('/simple-auth/admin/login')
    }
  }
  ...
  const loadDashboardStats = async () => {
    try {
      ...
      console.warn("[External Admin] Could not load stats:", error)
    }
  }

  const loadCurrentSession = () => {
    const savedSession = localStorage.getItem('current_external_session')
    const savedAttendees = localStorage.getItem('external_attendees')
  ...
  // Generate a QR code for external attendance
  const generateExternalQR = async () => {
    ...
    try {
      ...
      const qrCodeUrl = `${baseUrl}/api/external?sessionId=${sessionId}&token=${sessionId}`
      ...
      localStorage.setItem('current_external_session', JSON.stringify(newSession))
      localStorage.setItem('external_attendees', JSON.stringify([]))
      ...
      toast.success("External QR Code Generated!", {
  ...
  const endSession = async () => {
    ...
    localStorage.removeItem('current_external_session')
  ...
  // Navigate to grading management
  const goToGrading = () => {
    router.push('/external-attendance/grading')
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-foreground">External Attendance Admin</h1>
            <p className="text-muted-foreground">Generate QR codes for native camera scanning</p>
          </div>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              External Attendance Administrator
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Create QR codes that work with native camera apps
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <QrCode className="w-5 h-5" />
                External Attendance QR
              </CardTitle>
              <CardDescription>
                Students scan this with their phone's native camera
              </CardDescription>
            </CardHeader>
            <CardContent>
              {generatedQR ? (
                <div className="space-y-4">
                  <div className="flex justify-center">
                    <div className="p-4 bg-white rounded-lg border">
                      <img 
                        src={generatedQR} 
                        alt="External Attendance QR Code" 
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      Show this QR code to students. They can scan it with their phone's native camera app.
                    </AlertDescription>
                  </Alert>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center h-64 text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select a subject and academic level to generate an external QR code
                    </p>
                  </div>
                </div>
              )}
            ...
            <CardContent>
              {attendees.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Students will appear here after scanning the QR code with their native camera
                  </p>
                </div>
              )}
            </CardContent>
