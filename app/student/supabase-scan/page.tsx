...
/*
 * Simplified Supabase Student Scanner
 * A simplified version of the student QR scanner with Supabase integration
 */
...
export default function SupabaseStudentScanner() {
    ...
    const loadStudentData = async () => {
      try {
        ...
        if (localStudent) {
          ...
          console.log("[Supabase Scanner] Loaded student:", studentData.name)
          
          // Load attendance history from Supabase
        ...
        console.error("[Supabase Scanner] Error loading student:", error)
        router.push('/simple-auth/student/login')
      }
    }
    ...
    try {
      ...
      if (supabase) {
        ...
        if (!error && data) {
          ...
          console.log("[Supabase Scanner] Loaded attendance history from Supabase")
        }
      }
    } catch (error) {
      console.warn("[Supabase Scanner] Could not load attendance history:", error)
    }
  }
  ...
  const startCameraScanning = () => {
    ...
    try {
      ...
      const onScanSuccess = (decodedText: string) => {
        ...
        console.log("[Supabase Scanner] QR Code detected:", decodedText)
      ...
      const onScanFailure = (error: string) => {
        ...
        if (!error.includes("No QR code found")) {
          console.warn("[Supabase Scanner] Scan error:", error)
        }
      }
      ...
      console.error("[Supabase Scanner] Camera error:", error)
      setIsScanning(false)
  ...
  // Process QR code data with Supabase integration
  const processQRCode = async (qrData: string) => {
    try {
      ...
      if (supabase) {
        ...
        const { error } = await supabase
          ...
          .insert({
            ...
            student_id: student?.id || 'demo_student',
            student_name: student?.name || 'Demo Student',
      ...
      console.error("[Supabase Scanner] Error processing QR:", error)
      setScanResult({
        success: false,
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    html5QrCode.scanFile(file, true)
      .then(decodedText => {
        console.log("[Supabase Scanner] File QR detected:", decodedText)
        processQRCode(decodedText)
      })
      .catch(err => {
        console.error("[Supabase Scanner] File scan error:", err)
        setScanResult({
          success: false,
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Supabase QR Scanner</h1>
            <p className="text-muted-foreground">Scan QR codes to mark attendance with Supabase</p>
          </div>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Student Dashboard with Supabase Integration
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        {attendanceHistory.length > 0 && (
          <Card className="mt-6">
            <CardHeader>
              ...
              <CardDescription>
                Your last 5 attendance records from Supabase
              </CardDescription>
            </CardHeader>
            <CardContent>
